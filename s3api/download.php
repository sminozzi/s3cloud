      ...
      try {
        if ( headers_sent() ) {
          ...
          if ( gettype($result) !== 'object' ) {
              ob_end_flush();
              die( "File: ". __FILE__ ." Line: ". __LINE__ .  " Cannot dispatch file, Unable to get file from S3 cloud." );
          }
          header( 'Content-Description: File Transfer' );
          header( 'Content-Type: application/octet-stream' ); // http://stackoverflow.com/a/20509354
          header('Content-Disposition: attachment; filename="'.esc_attr(S3CLOUDFILENAME).'"');
          header( 'Expires: 0' );
